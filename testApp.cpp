...
void testApp::update(){
    ...
    if(danger == true && welcome == false){
        ...
        if (currentTime - startTime < totalTime){
            ...
            cout<<startTime<<endl;
            extreme = true;
            transparency.loadImage("images/alert.gif");
        }
        else {
            gameplay.close();
            gameover.update();
...
void testApp::draw(){
 ...
 //   }
  ...
  //  ofPopStyle();
    ...
    if (attention > 0.0)
    {
        ofSetColor(ofColor::yellow, ofMap(currAw, 0.0, ofGetWidth()/2, 50, 15));
        font.drawString(ofToString(attention), 10, ofGetHeight()/15 - 30);
    }
    ofPopStyle();
    ofPopMatrix();
